...
class TestDormandPrince89Solver:
    def test_solve_simple_ode(self):
        ...
        solver = DormandPrince89Solver(config)
        ...
        expected = np.exp(-1.0)
        assert abs(final_value - expected) < 1e-6  # Should be very accurate due to high order
